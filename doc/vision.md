...
- `/clear` - очистка истории диалога (бот "забудет" предыдущие сообщения)
...
### 2. Очистка истории диалога
Пользователь отправляет команду `/clear`:
1. Бот удаляет всю историю сообщений для этого `chat_id`
2. Бот отправляет подтверждение: "История диалога очищена. Можем начать с чистого листа!"
3. Следующие сообщения обрабатываются без контекста предыдущих

Это полезно, когда:
- Пользователь хочет сменить тему разговора
- Бот "запутался" в контексте
- Пользователь хочет начать новую консультацию

### 3. Нетекстовые сообщения
